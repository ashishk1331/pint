...
export const useStore = create((set) => ({
  ...
  gallery: [],
  setGallery: (item) => set(prev => ({ gallery: [ item, ...prev.gallery ] })),
  deleteGallery:(item) => set(prev => ({gallery:[]})),
  filterGallery:(id) => set(prev => ({gallery:prev.gallery.filter((item) => item.id!=id)})),
  // shadow value
  shadow: 'lg',
